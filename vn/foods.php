<?php
session_start();

if (!isset($_SESSION['user_id'])) {
    header('Location: login.php');
    exit;
}
?>

<!DOCTYPE html>
<html lang="vi">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>GymExcel - Thực phẩm</title>
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="../css/global.css">
  <link rel="stylesheet" href="../css/foods.css">
</head>
<body>
  <header>
    <div class="header-content">
      <a href="index.php" class="logo">
        <svg class="logo-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <path d="M6.5 6.5h.01M6.5 17.5h.01M17.5 6.5h.01M17.5 17.5h.01M19 12h.01M5 12h.01M12 2v4m0 12v4M8 4h8M8 20h8" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        GymExcel
      </a>
      <nav>
        <a href="index.php" class="nav-link">Bảng điều khiển</a>
        <a href="profile.php" class="nav-link">Cá nhân</a>
        <a href="foods.php" class="nav-link active">Thực phẩm</a>
        <a href="../backend/auth/logout.php" class="nav-link">Đăng xuất</a>
      </nav>
    </div>
  </header>

  <div class="container">
    <div class="card mb-3">
      <h1 style="font-size: 2rem; font-weight: 800; margin-bottom: 0.5rem;">Cơ sở dữ liệu thực phẩm</h1>
      <p style="color: var(--text-secondary);">Tìm kiếm và duyệt thông tin dinh dưỡng</p>
    </div>

    <div class="filters-section">
      <input
        type="text"
        class="search-input"
        id="searchInput"
        placeholder="Tìm kiếm thực phẩm..."
        onkeyup="filterFoods()"
      >
      <div class="filter-buttons">
        <button class="filter-btn active" data-filter="all" onclick="setFilter('all')">Tất cả</button>
        <button class="filter-btn" data-filter="protein" onclick="setFilter('protein')">Protein</button>
        <button class="filter-btn" data-filter="carbs" onclick="setFilter('carbs')">Tinh bột</button>
        <button class="filter-btn" data-filter="fat" onclick="setFilter('fat')">Chất béo</button>
        <button class="btn btn-primary" onclick="openAddFood()">+ Thêm thực phẩm</button>
      </div>
    </div>

    <div class="foods-grid" id="foodsGrid">
    </div>

    <div class="empty-state hidden" id="emptyState">
      <svg class="empty-state-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <circle cx="12" cy="12" r="10"/>
        <path d="M12 6v6l4 2"/>
      </svg>
      <div class="empty-state-title">Không tìm thấy thực phẩm</div>
      <div class="empty-state-description">Hãy thử thay đổi từ khóa hoặc bộ lọc</div>
    </div>
  </div>

  <div class="modal hidden" id="addFoodModal">
    <div class="modal-content">
      <h2>Thêm thực phẩm</h2>
      <form id="addFoodForm">
        <label>Tên thực phẩm</label>
        <input type="text" id="foodName" placeholder="Ức gà" required>

        <label>Danh mục</label>
        <select id="foodCategory" required>
          <option value="">Chọn danh mục</option>
          <option value="protein">Protein</option>
          <option value="carbs">Tinh bột</option>
          <option value="fat">Chất béo</option>
        </select>

        <label>Khẩu phần (g)</label>
        <input
          type="text"
          id="foodServing"
          placeholder="100g"
          value="100"
        >

        <label>Lượng calo (mỗi khẩu phần)</label>
        <input type="number" id="foodCalories" placeholder="Calo" min="1" required>

        <label>Protein (g)</label>
        <input type="number" id="foodProtein" min="0" required>

        <label>Tinh bột (g)</label>
        <input type="number" id="foodCarbs" min="0" required>

        <label>Chất béo (g)</label>
        <input type="number" id="foodFat" min="0" required>

        <div class="modal-actions">
          <button class="btn-cancel" type="button" onclick="closeAddFood()">Hủy</button>
          <button class="btn-primary btn-lg" id="save_form" type="submit">Thêm</button>
        </div>
      </form>
    </div>
  </div>

  <script src="./js/data.js"></script>
  <script src="./js/foods.js"></script>
</body>
</html>
